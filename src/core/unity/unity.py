...
class PlayerWrapper:
    ...
    def __handle_input_data(
        ...
        lines = ["{time_in}#{text}\n".format(**data) for data in iput_data]
    ...
    def __start_frames_capture(
        ...
        try:
            ...
            self.__logger.debug("{} successfully generated.".format(frames_path))

        except OSError as ex:
            if ex.errno == errno.ENOENT:
                self.__logger.exception("'{0}' not found.".format(*unity_cmd))
            else:
                self.__logger.exception("'{0}' spawn failed.".format(*unity_cmd))

        except subprocess.CalledProcessError:
            self.__logger.exception("'{0}' returned a non-zero exit code.".format(*unity_cmd))
    ...
    def run(
        self,
        subtitle_blocks: dict,
        correlation_tag: str,
        options: dict = {}
    ) -> tuple:

        gloss_file = self.__handle_input_data(subtitle_blocks, correlation_tag)
        libras_frames = self.__start_frames_capture(gloss_file, correlation_tag, options)
        libras_video = self.__start_video_rendering(libras_frames, correlation_tag)
        ...
        return  libras_video, libras_video_duration
